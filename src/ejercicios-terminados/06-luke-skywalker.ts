...
import { switchMap, map, pluck, concatMap } from 'rxjs/operators';
...
(() => {
    ...
    getRequest(`${SW_API}/people/1/`).pipe(
        ...
        pluck<any, string[]>('starships'),
        concatMap<string[], any>(([urlStarship1]) => {
            return getRequest(`${urlStarship1}`)
        })
    ...
    getRequest(`${SW_API}/people/1/`).pipe(
        ...
        concatMap(resp =>
            zip(
                of(resp),
                getRequest(resp.starships[0])
            )
        ),
        map(
            ([people, starship])=>({people, starship})
        )
